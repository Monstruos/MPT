...
class ConverterTest {
    private static final double EPSILON = Math.pow(0.1, Converter.maxPrecisionForBase(10) - 1);

    @Test
    void whenZeroInCorrectBaseThenStringValueIsZero() {
        for (int base = Converter.MIN_BASE; base <= Converter.MAX_BASE; ++base) {
            for (int precision = 0; precision <= Converter.maxPrecisionForBase(base); ++precision) {
    ...
    void whenDecimalIntegerThenStringValueIsThatInteger() {
        for (int number = 1; number < Converter.MAX_BASE * 2; ++number) {
            for (int precision = 0; precision <= Converter.maxPrecisionForBase(10); ++precision) {
    ...
    void whenDoubleValueToStringAndToDoubleThenDoubleValue() {
        for (double number = 0.1; number < Converter.MAX_BASE; number += 0.1) {
            double convertedValue = Converter.convertToDouble(Converter.convertToString(number, 10, Converter.maxPrecisionForBase(10)), 10);
            double diff = Math.abs(number - convertedValue);

            assertTrue(diff < EPSILON, number + " is not equal to " + convertedValue + " diff: " + diff);
        }
    }
